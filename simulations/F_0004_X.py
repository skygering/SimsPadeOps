...
grid_convergence_change_filter = itertools.product(itertools.zip_longest(cT, tstop_CT1), itertools.zip_longest(nx, ny, nz, dt, filterWidth, [True] * len(nx)))
# constant filter width (0.08) + corrected
grid_convergence_const_filter = itertools.product(itertools.zip_longest(cT, tstop_CT1), itertools.zip_longest(nx, ny, nz, dt, [0.08] * len(nx), [True] * len(nx)))
# constant filter width (0.08) + NOT corrected
grid_convergence_const_filter_no_correction = itertools.product(itertools.zip_longest(cT, tstop_CT1), itertools.zip_longest(nx, ny, nz, dt, [0.08] * len(nx), [False] * len(nx)))
